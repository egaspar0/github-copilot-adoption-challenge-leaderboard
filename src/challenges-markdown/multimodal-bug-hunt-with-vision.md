...
Install the latest **GitHub Copilot** extension in VS Code and enable both Agent mode and Vision under **Settings > GitHub Copilot**. Ensure your repository contains the Figma design export for the affected screen for example `design/login-page.png`. Start your development server with `npm start` `yarn dev` or similar so you can reproduce the bug.
